...
import { activeBuses, busLogs, summaryStats } from "../data/sbmsData";
...
export default function Homepage() {
  const latestLogs = busLogs.slice(0, 3);
          ...
          <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1.1fr_0.9fr]">
            <div>
              ...
              <h1 className="mt-6 max-w-3xl text-5xl font-semibold tracking-tight text-slate-950 sm:text-6xl">
                Replace manual gate registers with one operational frontend.
              </h1>
              <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600">
                This SBMS frontend models the two core workflows from the project idea: security guards log arrivals and
                departures quickly, while administrators monitor fleet status, alerts, and reporting from a central dashboard.
              </p>
              ...
              <div className="mt-10 grid gap-4 sm:grid-cols-3">
                <Card className="gap-3 border-slate-200 bg-white/85 p-5 shadow-sm">
                  <p className="text-sm font-medium text-slate-500">Live buses</p>
                  <p className="text-3xl font-semibold text-slate-900">{activeBuses.length}</p>
                  <p className="text-sm text-slate-600">Currently on campus</p>
                </Card>
                <Card className="gap-3 border-slate-200 bg-white/85 p-5 shadow-sm">
                  <p className="text-sm font-medium text-slate-500">Trips today</p>
                  <p className="text-3xl font-semibold text-slate-900">{summaryStats[0].value}</p>
                  <p className="text-sm text-slate-600">Across all campus gates</p>
                </Card>
                <Card className="gap-3 border-slate-200 bg-white/85 p-5 shadow-sm">
                  <p className="text-sm font-medium text-slate-500">Recent alerts</p>
                  <p className="text-3xl font-semibold text-slate-900">3</p>
                  <p className="text-sm text-slate-600">Capacity and delay cases</p>
                </Card>
              </div>
            </div>
            ...
            <Card className="overflow-hidden border-slate-200 bg-slate-950 text-white shadow-2xl shadow-slate-900/20">
              ...
              <div className="grid gap-4 px-6 py-6">
                {activeBuses.slice(0, 3).map((bus) => (
                  <div key={bus.busNumber} className="rounded-2xl border border-white/10 bg-white/5 p-4">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      >
                        {bus.status === "overcrowded" ? "Capacity risk" : "Inside campus"}
                      </span>
                    </div>
                    <div className="mt-4 flex flex-wrap gap-4 text-sm text-slate-300">
                      <span>Guard: {bus.guard}</span>
                      <span>Location: {bus.location}</span>
                      <span>Occupancy: {bus.occupancy}/{bus.capacity}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <section id="modules" className="px-6 py-18">
          <div className="mx-auto max-w-7xl">
            <div className="max-w-3xl">
              ...
              <h2 className="mt-3 text-4xl font-semibold tracking-tight text-slate-950">Frontend aligned to the project idea</h2>
              <p className="mt-4 text-lg text-slate-600">
                The current pass focuses on the flows that matter first: secure logging for guards and an overview dashboard for transport administration.
              </p>
            </div>
            ...
            <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
              <Card className="border-slate-200 bg-white p-8 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-2xl font-semibold text-slate-950">Why this cleanup matters</h3>
                </div>
                <div className="mt-6 space-y-4 text-slate-600">
                  <p>The previous frontend was visually appealing but disconnected from the real SBMS workflow.</p>
                  <p>This version keeps the mock-only scope but organizes the app around the actual project requirements: guard logging, admin monitoring, and actionable alerts.</p>
                  <p>It also removes broken copy artifacts and replaces scattered hardcoded component data with one shared source.</p>
                </div>
              </Card>
              ...
              <Card className="border-slate-200 bg-white p-8 shadow-sm">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-2 text-sm text-slate-600">Representative log history for the frontend prototype.</p>
                  </div>
